    ...
    for file in os.listdir(commandsDirectory):
        ...
        if fileExtension == '.py':
            module = imp.load_source(
                fileName, os.path.join(commandsDirectory, file))
...
def loadCommand(module, command, directory, filename, extension):
    func = makeRunCommand(
        command, os.path.join(directory, filename + extension))
    name = command.name()
    ...
    lldb.debugger.HandleCommand(
        'script ' + functionName + ' = sys.modules[\'' + module.__name__ + '\']._loadedFunctions[\'' + key + '\']')
    lldb.debugger.HandleCommand(
        'command script add -f ' + functionName + ' ' + name)
...
def makeRunCommand(command, filename):
    def runCommand(debugger, input, result, dict):
        ...
        # the initial args form an expression and combine them into a single
        # arg.
...
def helpForCommand(command, filename):
    ...
    help += '\n\nThis command is implemented as %s in %s.' % (
        command.__class__.__name__, filename)
