...
def maskView(viewOrLayer, color, alpha):
  ...
  window = ch.evaluateExpression('(UIWindow *)[[UIApplication sharedApplication] keyWindow]')
  origin = convertPoint(0, 0, viewOrLayer, window)
  size = ch.evaluateExpressionValue('(CGSize)((CGRect)[(id)%s frame]).size' % viewOrLayer)
  ...
  mask = ch.evaluateExpression('[((UIView *)[UIView alloc]) initWithFrame:%s]' % rectExpr)
...
def unmaskView(viewOrLayer):
  window = ch.evaluateExpression('(UIWindow *)[[UIApplication sharedApplication] keyWindow]')
  mask = ch.evaluateExpression('(UIView *)[%s viewWithTag:(NSInteger)%s]' % (window, viewOrLayer))
...
def convertPoint(x, y, fromViewOrLayer, toViewOrLayer):
  ...
  return ch.evaluateExpressionValue('(CGPoint)[%s convertPoint:(CGPoint){ .x = %s, .y = %s } toLayer:(CALayer *)%s]' % (fromLayer, x, y, toLayer))

def convertToLayer(viewOrLayer):
  if ch.evaluateBooleanExpression('[(id)%s isKindOfClass:(Class)[CALayer class]]' % viewOrLayer):
    return viewOrLayer
  elif ch.evaluateBooleanExpression('[(id)%s respondsToSelector:(SEL)@selector(layer)]' % viewOrLayer):
    return ch.evaluateExpression('(CALayer *)[%s layer]' % viewOrLayer)
  else:
