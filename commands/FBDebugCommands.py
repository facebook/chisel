...
def lldbcommands():
  return [
    WatchInstanceVariableCommand(),
    MethodBreakpointCommand(),
  ]

class WatchInstanceVariableCommand(ch.Command):
  def name(self):
    return 'wivar'
  ...
  def args(self):
    return [
      ...
      ch.CommandArgument(arg='ivarName', help='Name of the instance variable to watch.')
    ]

  def run(self, arguments, options):
    commandForObject, ivarName = arguments

    objectAddress = int(ch.evaluateObjectExpression(commandForObject), 0)

    ivarOffsetCommand = '(ptrdiff_t)ivar_getOffset((void *)object_getInstanceVariable((id){}, "{}", 0))'.format(objectAddress, ivarName)
    ivarOffset = ch.evaluateIntegerExpression(ivarOffsetCommand)
    ...
    ivarSize = int(ch.evaluateExpression(ivarSizeCommand), 0)
...
class MethodBreakpointCommand(ch.Command):
  def name(self):
    return 'bmessage'
  ...
  def args(self):
    return [
      ch.CommandArgument(arg='expression', type='string', help='Expression to set a breakpoint on, e.g. "-[MyView setFrame:]", "+[MyView awesomeClassMethod]" or "-[0xabcd1234 setFrame:]"'),
    ]
  ...
  def run(self, arguments, options):
    ...
    targetIsClass = False
    targetObject = ch.evaluateObjectExpression('({})'.format(classNameOrExpression), False)
    ...
    if not targetObject:
      ...
      targetIsClass = True
      targetObject = ch.evaluateObjectExpression('[{} class]'.format(classNameOrExpression), False)

    targetClass = ch.evaluateObjectExpression('[{} class]'.format(targetObject), False)
