...
class FBPrintInvocation(fb.FBCommand):
    ...
    def options(self):
        return [
            fb.FBCommandArgument(short='-a', long='--all', arg='all', default=False, boolean=True,
...
def printInvocationForFrame(frame):
    ...
    commandForSignature = '[(id)' + self + \
        ' methodSignatureForSelector:(char *)sel_getName(' + cmd + ')]'
    ...
    commandForInvocation = '[NSInvocation _invocationWithMethodSignature:(id)' + \
        signature + ' frame:((void *)' + str(arg0) + ')]'
...
def stackStartAddressInSelectedFrame(frame):
    # Determine if the %ebp register has already had the stack register pushed
    # into it (always the first instruction)
...
def prettyPrintInvocation(frame, invocation):
    ...
    selector = fb.evaluateExpressionValue(
        '(char *)sel_getName((SEL)[(id)' + invocation + ' selector])').GetSummary()
    selector = re.sub(r'^"|"$', '', selector)

    objectClassValue = fb.evaluateExpressionValue(
    ...
    if len(argDescriptions) > 0:
        ...
        for argDescription in argDescriptions:
            ...
            lldb.debugger.HandleCommand(
                'expr (void)[' + invocation + ' getArgument:&$v atIndex:' + str(index) + ']')
            ...
            if readableString:
                ...
                print (hex(address) + ', address of ' +
                       encoding + ' ' + description).strip()
