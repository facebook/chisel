    ...
    def run(self, arguments, options):
        if options.upwards:
            ...
            lldb.debugger.HandleCommand(
                'po (id)[' + arguments[0] + ' recursiveDescription]')
...
class FBPrintCoreAnimationTree(fb.FBCommand):
    ...
    def run(self, arguments, options):
        lldb.debugger.HandleCommand(
            'po [NSString stringWithCString:(char *)CARenderServerGetInfo(0, 2, 0)]')
...
def tableViewInHierarchy():
    viewDescription = fb.evaluateExpressionValue(
        '(id)[(id)[UIWindow keyWindow] recursiveDescription]').GetObjectDescription()
...
class FBPrintInternals(fb.FBCommand):
    ...
    def run(self, arguments, options):
        ...
        objectClass = fb.evaluateExpressionValue(
            '(id)[(id)(' + object + ') class]').GetObjectDescription()
...
class FBPrintInstanceVariable(fb.FBCommand):
    ...
    def run(self, arguments, options):
        ...
        lldb.debugger.HandleCommand(
            '{} (({} *)({}))->{}'.format(printCommand, objectClass, object, ivarName))
