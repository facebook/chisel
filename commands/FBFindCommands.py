...
class FBFindViewControllerCommand(fb.FBCommand):
    ...
    def options(self):
        return [
            fb.FBCommandArgument(short='-n', long='--name', arg='classNameRegex', type='string',
                                 help='The view-controller-class regex to search the view controller hierarchy for.'),
            fb.FBCommandArgument(short='-v', long='--view', arg='view', type='UIView',
    ...
    def run(self, arguments, options):
        if options.classNameRegex and options.view:
            ...
            output = vcHelpers.viewControllerRecursiveDescription(
                '(id)[[UIWindow keyWindow] rootViewController]')
            searchString = options.classNameRegex if options.classNameRegex else arguments[
                0]
    ...
    def isViewController(object):
        command = '[(id){} isKindOfClass:[UIViewController class]]'.format(
            object)
...
class FBFindViewCommand(fb.FBCommand):
    ...
    def run(self, arguments, options):
        output = fb.evaluateExpressionValue(
            '(id)[[UIWindow keyWindow] recursiveDescription]').GetObjectDescription()
        printMatchesInViewOutputStringAndCopyFirstToClipboard(
            arguments[0], output)
...
def printMatchesInViewOutputStringAndCopyFirstToClipboard(needle, haystack):
    ...
    for match in re.finditer('.*<.*(' + needle + ').*: (0x[0-9a-fA-F]*);.*', haystack, re.IGNORECASE):
        ...
        className = fb.evaluateExpressionValue(
            '(id)[(' + view + ') class]').GetObjectDescription()
...
class FBFindViewByAccessibilityLabelCommand(fb.FBCommand):
    ...
    def run(self, arguments, options):
        first = None
        haystack = fb.evaluateExpressionValue(
            '(id)[[UIWindow keyWindow] recursiveDescription]').GetObjectDescription()
        needle = arguments[0]

        allViews = re.findall('.* (0x[0-9a-fA-F]*);.*', haystack)
        for view in allViews:
            a11yLabel = fb.evaluateExpressionValue(
                '(id)[(' + view + ') accessibilityLabel]').GetObjectDescription()
...
class FBTapLoggerCommand(fb.FBCommand):
    ...
    def run(self, arguments, options):
        ...
        breakpoint.SetCondition(
            '(int)[' + parameterExpr + ' type] == 0 && (int)[[[' + parameterExpr + ' allTouches] anyObject] phase] == 0')
        breakpoint.SetOneShot(True)
        lldb.debugger.HandleCommand(
            'breakpoint command add -s python -F "sys.modules[\'' + __name__ + '\'].' + self.__class__.__name__ + '.taplog_callback" ' + str(breakpoint.id))
    ...
    def taplog_callback(frame, bp_loc, internal_dict):
        parameterExpr = objc.functionPreambleExpressionForObjectParameterAtIndex(
            0)
        lldb.debugger.HandleCommand(
            'po [[[%s allTouches] anyObject] view]' % (parameterExpr))
        # We don't want to proceed event (click on button for example), so we
        # just skip it
